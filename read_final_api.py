...
parser.add_argument('--video', default='output2_video.mp4')
...
def draw_colored_regions(frame):
    """
    Vẽ và tô màu các vùng dựa trên số lượng đối tượng trong mỗi vùng.
    """
    for region_name, points in regions.items():
        total = total_counts[region_name]

        # Xác định màu sắc cho vùng
        if total >= 10:
            color = (0, 0, 255)  # Đỏ (BGR)
        elif total >= 5:
            color = (0, 255, 255)  # Vàng (BGR)
        else:
            continue
        # Vẽ đường viền và tô màu vùng
        cv2.polylines(frame, [np.array(points, np.int32)], isClosed=True, color=color, thickness=2)
        cv2.fillPoly(frame, [np.array(points, np.int32)], color=color)

        # Hiển thị tên vùng và tổng số đối tượng trong vùng
        position = points[0]  # Lấy tọa độ đầu tiên làm điểm hiển thị text
        cv2.putText(
            frame, f"{region_name}: {total}", position,
...
def generate_frames():
    """
    Tạo các khung hình từ webcam và phát trực tuyến qua Flask.
    """
    ...
    while True:
        ...
        # Vẽ và tô màu các vùng trên khung hình

        # Mã hóa khung hình thành JPEG để phát trực tuyến
        ret, buffer = cv2.imencode('.jpg', frame)
        frame = buffer.tobytes()

        # Tạo HTTP response stream cho Flask
        yield (b'--frame\r\n'
